...
function Dashboard() {
    ...
    useEffect(()=>{
        if(credentials.adminId!=""){
            setURLs({auth:"http://localhost:3000/oauth/"+credentials.adminId,token:"http://localhost:5000/verify_token/"+credentials.adminId})
        }
    },[credentials])
    useEffect(()=>{
        ...
        <main className="flex min-h-screen w-full flex-col gap-3 items-start justify-start p-4 py-8 md:p-24 bg-slate-200">
            ...
            <div className="md:w-1/2 w-full">
                ...
                <div className="flex gap-2">
                    <input  id="name" className="bg-gray-50 border border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " disabled value={window.location.protocol+"//"+window.location.host+"/login"} />
                    <button onClick={()=>copyCredential(window.location.host+"/login")} className="flex items-center justify-center py-4 bg-slate-700 px-3 py-2 text-white">
